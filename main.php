...
// --- Simple metrics and live JSON endpoints ---
...
// Early lightweight JSON endpoints (avoid incrementing counters on polling)
if (isset($_GET['action']) && in_array($_GET['action'], ['active_users_json','metrics_json','active_users_remove'], true)) {
    $now = time();
    $active_map = [];
    ...
    $resp = [];
    if ($_GET['action'] === 'active_users_remove') {
        // Remove this session from the active users map immediately
        unset($active_map[session_id()]);
        @file_put_contents($activeFile, json_encode($active_map));
        $resp = [ 'ok' => true, 'active_users' => count($active_map) ];
        header('Content-Type: application/json');
        echo json_encode($resp);
        exit;
    }

    // Update current session timestamp and prune old sessions (for read endpoints)
    $active_map[session_id()] = $now;
    $window = 5 * 60;
    foreach ($active_map as $sid => $ts) {
        if ($now - (int)$ts > $window) unset($active_map[$sid]);
    }
    @file_put_contents($activeFile, json_encode($active_map));
    $resp = [ 'active_users' => count($active_map) ];
    if ($_GET['action'] === 'metrics_json') {
        // Read without incrementing
        if (!file_exists($visitsFile)) { @file_put_contents($visitsFile, '0'); }
        $total_visits_read = (int)@file_get_contents($visitsFile);
        $today = date('Y-m-d');
        $daily_map = [];
        ...
        $today_visits_read = (int)($daily_map[$today] ?? 0);
        $resp['total_visits'] = $total_visits_read;
        $resp['today_visits'] = $today_visits_read;
    }
    header('Content-Type: application/json');
    echo json_encode($resp);
    exit;
...
// --- Simple metrics: total visits and active users (page render path) ---
...
if (file_exists($dailyFile)) {
    $rawDaily = @file_get_contents($dailyFile);
    $decodedDaily = @json_decode($rawDaily, true);
    if (is_array($decodedDaily)) {
        $daily_map = $decodedDaily;
    }
}
if (!isset($daily_map[$today])) {
    $daily_map[$today] = 0;
}
...
// Support lightweight polling endpoint for live active user count
if (isset($_GET['action']) && $_GET['action'] === 'active_users_json') {
    // Update current session timestamp
    $active_map[session_id()] = $now;
    // Prune old sessions (5 minutes window)
    $window = 5 * 60;
    foreach ($active_map as $sid => $ts) {
        if ($now - (int)$ts > $window) unset($active_map[$sid]);
    }
    @file_put_contents($activeFile, json_encode($active_map));
    header('Content-Type: application/json');
    echo json_encode([ 'active_users' => count($active_map) ]);
    exit;
}
...
<head>
    ...
    <style>
        ...
        /* Active users bump animation */
        @keyframes countBump {
            0% { transform: scale(1); color: inherit; }
            30% { transform: scale(1.12); color: #1f6a45; }
            100% { transform: scale(1); color: inherit; }
        }
        .count-bump {
            animation: countBump 400ms ease-in-out;
        }
...
<body>
                ...
                <div class="lang-menu" id="langMenu">
                    <button class="lang-trigger" id="langTrigger" aria-haspopup="true" aria-expanded="false">
                        <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
                            <path fill="currentColor" d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 2c.9 0 1.75.19 2.52.54-.3.38-.58.84-.83 1.36H10.3A7.98 7.98 0 0 1 12 4Zm-2.34 2h4.68c-.2.6-.37 1.26-.49 2H10.15c.12-.74.29-1.4.5-2ZM6.1 8h2.86c-.08.64-.12 1.31-.12 2H5.52c.12-.71.33-1.38.58-2Zm-1 4h3.84c.02.69.07 1.36.15 2H5.52c-.18-.64-.3-1.31-.42-2Zm.94 4h2.97c.24.92.57 1.73.96 2.37A8.03 8.03 0 0 1 6.04 16ZM12 20c-1.26 0-2.42-1.57-3.06-4h6.12c-.64 2.43-1.8 4-3.06 4Zm2.99-1.63c.39-.64.72-1.45.96-2.37h2.97a8.03 8.03 0 0 1-3.93 2.37ZM18.48 14h-3.84c.08-.64.13-1.31.15-2h3.84c-.12.69-.24 1.36-.15 2Zm.15-4h-3.42c0-.69-.04-1.36-.12-2h2.86c.25.62.43 1.29.58 2ZM12 4c1.26 0 2.42 1.57 3.06 4h-6.12C9.58 5.57 10.74 4 12 4Z" />
                        </svg>
                        <span id="currentLangLabel">English</span>
                        <svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" class="chevron">
    ...
    <section class="hero-banner">
        <div class="hero-overlay">
            ...
            <h1 class="hero-title" id="i18n-title" data-i18n="title" style="text-align: center;">LGU OF ROXAS: INFOCHAT</h1>
        </div>
    </section>

    <section class="quick-actions">
        <a class="qa" href="edit_profile.php">
            <span class="qa-icon"><svg viewBox="0 0 24 24" aria-hidden="true">
                ...
                <section class="site-stats" aria-label="Site usage statistics">
                    ...
                    <div class="stats-grid stats-grid-3">
                        <div class="stat-card">
                            ...
                            <div class="stat-body">
                                <div class="stat-number" id="total-visits-count"><?php echo number_format($total_visits); ?></div>
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-body">
                                <div class="stat-number" id="today-visits-count"><?php echo number_format($today_visits); ?></div>
                            ...
                            <div class="stat-top">
                                <span class="stat-icon" aria-hidden="true">
                                    <svg viewBox="0 0 24 24" width="28" height="28">
                                        <path fill="currentColor" d="M16 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm-8 0a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm0 2c-2.67 0-8 1.34-8 4v3h16v-3c0-2.66-5.33-4-8-4Zm8 0c-.34 0-.7.02-1.06.05 1.63.77 3.06 1.97 3.06 3.95v3h6v-3c0-2.66-5.33-4-8-4Z" />
                                    </svg>
                                </span>
                            </div>
                            <div class="stat-body">
                                <div class="stat-number" id="active-users-count"><?php echo number_format($active_users); ?></div>
                                    ...
                                    <div class="card-social">
                                        <a href="https://www.facebook.com/rvdulatre.20" class="social-link" aria-label="Facebook" target="_blank" rel="noopener">
                                            <svg viewBox="0 0 24 24" width="20" height="20">
                                                <path fill="currentColor" d="M22 12a10 10 0 1 0-11.5 9.9v-7h-2v-3h2v-2.3c0-2 1.2-3.1 3-3.1 .9 0 1.8.1 1.8.1v2h-1c-1 0-1.3.6-1.3 1.2V12h2.3l-.4 3h-1.9v7A10 10 0 0 0 22 12" />
                                            </svg>
                                        </a>
                ...
                <div class="social">
                    <a href="mailto:info@example.com" aria-label="Email">
                        <svg viewBox="0 0 24 24" aria-hidden="true">
                            <path d="M4 4h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm0 2l8 5 8-5" />
                        </svg>
                    </a>
    ...
    <script defer>
        (function() {
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const dict = {
                    en: {
                        ...
                        by_the_numbers: 'InfoChat By The Numbers',
                        total_visits: 'Total visits',
                        todays_visits: "Today's visits",
                        active_users_5min: 'Active users (5 min)',
                        ...
                        logout: 'Log Out',
                        greeting: 'Hi',
                        welcome_dashboard: 'Welcome to InfoChat.',
                        find_services: 'Find services and tools fast',
                        find_services_desc: 'Search, navigate quick links, or explore the latest updates.',
                        ql_edit_profile: 'Edit Profile',
                        ql_edit_profile_desc: 'Update your username and details',
                        ql_feedback: 'Feedback Survey',
                        ql_feedback_desc: 'Tell us how we can improve',
                        ql_backup: 'Backup',
                        ql_backup_desc: 'Create a database backup',
                        ql_restore: 'Restore',
                        ql_restore_desc: 'Restore from a backup file',
                        ql_admin: 'Admin Panel',
                        ql_admin_desc: 'Manage users and data',
                        logout_desc: 'End your session securely',
                        latest_news: 'Latest News',
                        upcoming_events: 'Upcoming Events',
                        search_placeholder: 'How can we help you?',
                        hero_placeholder: 'Search InfoHUB (e.g., backup, restore, edit profile)'
                    },
                    fil: {
                        welcome: 'MALIGAYANG PAGDATING SA',
                        title: 'LGU NG ROXAS: INFOCHAT',
                        by_the_numbers: 'InfoChat Sa Mga Numero',
                        total_visits: 'Kabuuang pagbisita',
                        todays_visits: 'Pagbisita ngayon',
                        active_users_5min: 'Aktibong user (5 minuto)',
                        edit_profile: 'I-edit ang Profile',
                        give_feedback: 'Magbigay ng Feedback',
                        backup: 'Backup',
                        restore: 'Ibalik',
                        admin: 'Admin',
                        meetings: 'Mga Pulong',
                        logout: 'Mag-logout',
                        greeting: 'Kamusta',
                        welcome_dashboard: 'Maligayang pagdating sa iyong InfoChat dashboard.',
                        find_services: 'Hanapin ang mga serbisyo at tool nang mabilis',
                        find_services_desc: 'Maghanap, gumamit ng mga mabilisang link, o tingnan ang mga pinakabagong update.',
                        ql_edit_profile: 'I-edit ang Profile',
                        ql_edit_profile_desc: 'I-update ang iyong username at detalye',
                        ql_feedback: 'Feedback Survey',
                        ql_feedback_desc: 'Sabihin sa amin kung paano pa namin mapapabuti',
                        ql_backup: 'Backup',
                        ql_backup_desc: 'Gumawa ng database backup',
                        ql_restore: 'Ibalik',
                        ql_restore_desc: 'Ibalik mula sa backup file',
                        ql_admin: 'Admin Panel',
                        ql_admin_desc: 'Pamahalaan ang mga user at data',
                        logout_desc: 'Tapusin ang iyong session nang ligtas',
                        latest_news: 'Pinakabagong Balita',
                        upcoming_events: 'Mga Nalalapit na Kaganapan',
                        search_placeholder: 'Paano ka namin matutulungan?',
                        hero_placeholder: 'Maghanap sa InfoChat (hal., backup, restore, edit profile)'
                    }
                };
                ...
                applyLang(savedLang);
                if (langBtn) {
                    langBtn.addEventListener('change', e => applyLang(e.target.value));
                }
            ...
            window.VG_CONFIG = {
                ID: "XjyQcuAkSVYOg4pp", // YOUR AGENT ID
                ...
                userID: <?php echo json_encode($_SESSION['user_username']); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Live update for metrics without page refresh
            ...
            const todayVisitsEl = document.getElementById('today-visits-count');
            let lastActiveUsersValue = Number((activeUsersEl && activeUsersEl.textContent || '0').replace(/\D/g,'')) || 0;
            let lastTotalVisitsValue = Number((totalVisitsEl && totalVisitsEl.textContent || '0').replace(/\D/g,'')) || 0;
            let lastTodayVisitsValue = Number((todayVisitsEl && todayVisitsEl.textContent || '0').replace(/\D/g,'')) || 0;
            function animateCount(el, from, to, duration = 400){
                if (!el) return;
                if (to <= from) { el.textContent = to.toLocaleString(undefined); return; }
            ...
            async function refreshMetrics(){
                try {
                    const res = await fetch('main.php?action=metrics_json', { credentials: 'same-origin', cache: 'no-store' });
                    if (!res.ok) return;
                    const data = await res.json();
                    if (typeof data.active_users === 'number' && activeUsersEl) {
                        const newVal = data.active_users;
                        if (newVal > lastActiveUsersValue) {
                            bump(activeUsersEl);
                            animateCount(activeUsersEl, lastActiveUsersValue, newVal);
                        } else if (newVal !== lastActiveUsersValue) {
                            // no animation for decrease/equal, just set formatted
                            activeUsersEl.textContent = newVal.toLocaleString(undefined);
                        }
                        lastActiveUsersValue = newVal;
                    }
                    if (typeof data.total_visits === 'number' && totalVisitsEl) {
                        const newVal = data.total_visits;
                        if (newVal > lastTotalVisitsValue) {
                            bump(totalVisitsEl);
                            animateCount(totalVisitsEl, lastTotalVisitsValue, newVal);
                        } else if (newVal !== lastTotalVisitsValue) {
                            totalVisitsEl.textContent = newVal.toLocaleString(undefined);
                        }
                        lastTotalVisitsValue = newVal;
                    }
                    if (typeof data.today_visits === 'number' && todayVisitsEl) {
                        const newVal = data.today_visits;
                        if (newVal > lastTodayVisitsValue) {
                            bump(todayVisitsEl);
                            animateCount(todayVisitsEl, lastTodayVisitsValue, newVal);
                        } else if (newVal !== lastTodayVisitsValue) {
                            todayVisitsEl.textContent = newVal.toLocaleString(undefined);
                        }
                        lastTodayVisitsValue = newVal;
                    }
                } catch(e) {
                    // swallow errors silently
                }
            }
            // initial fetch soon after load, then every 5s
            ...
            // On tab/window close, signal immediate removal from active users
            try {
                const url = 'main.php?action=active_users_remove';
                if (navigator.sendBeacon) {
                    const data = new Blob([], { type: 'text/plain' });
                    navigator.sendBeacon(url, data);
                } else {
                    // Fallback best effort (may be canceled by the browser)
