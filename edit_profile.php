...
// Prevent guest accounts from accessing profile edits
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === 'update_username') {
        ...
        if ($new_password === '' || $current_password === '') {
            ...
            $fetch_stmt->execute();
            $fetch_result = $fetch_stmt->get_result(); // buffer result to avoid commands out of sync
            if ($row = $fetch_result->fetch_assoc()) {
                ...
                if (!password_verify($current_password, $hashed_password)) {
                    ...
                    if ($upd_stmt->execute()) {
                        // Optionally you might want to destroy session here
